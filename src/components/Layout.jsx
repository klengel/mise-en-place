...
import { BookOpen, CalendarDays, Calendar, Sparkles, Settings, ChefHat, LogOut, ClipboardList, FolderOpen, Package, ShoppingCart, ShoppingBag } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-60 bg-card border-r border-border shrink-0">
            ...
            if (item.divider) {
              return (
                <div key={i} className="pt-3 pb-1 px-3">
                  <p className="text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider">{item.label}</p>
                </div>
              );
            }
        ...
        <div className="px-3 py-3 border-t border-border">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg">
            ...
            <button onClick={signOut} title="Sign out" className="text-muted-foreground hover:text-foreground transition-colors shrink-0">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border flex justify-around px-1 py-2 overflow-x-auto">
        {navItems.filter(i => !i.divider).map(({ to, icon: Icon, label }) => (
        ...
        <button onClick={signOut} className="flex flex-col items-center gap-1 px-2 py-1 rounded-lg text-xs text-muted-foreground shrink-0">
